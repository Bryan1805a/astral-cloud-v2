...
import { restartServer, InvalidStateError, ServerLockedError } from "@/lib/services/server.service";
...
export async function POST(
  ...
  try {
    const server = await restartServer(params.serverId, payload.sub, ip);
    ...
    if (error instanceof InvalidStateError) {
      return apiError("INVALID_STATE", "This server cannot be restarted because it is not currently running.");
    }
