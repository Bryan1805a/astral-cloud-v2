...
Trong quá trình phát triển, toàn bộ ngăn xếp chạy bên trong Docker containers. Không cần cài đặt gì trên máy chủ (Node.js, npm, pnpm, Python) — chỉ cần Docker Engine và Git trên máy chủ. Tất cả tiến trình ứng dụng, dịch vụ hạ tầng, và công cụ đều thực thi bên trong containers, tuân theo triết lý phát triển Docker-first của dự án.
...
|-------------------|---------------------------|-----------------------------------------------------------------------------------------------|
| Next.js Web App   | Docker container          | `docker compose up`, cổng 3000. Mã nguồn được mount dưới dạng volume để hot-reload.           |
| BullMQ Worker     | Docker container          | Dịch vụ riêng trong file compose. Mã nguồn được mount dưới dạng volume để hot-reload.         |
...
| Customer          | Docker Engine             | Được tạo bởi worker — cùng daemon, cách ly bởi container. Worker container mount Docker socket (`/var/run/docker.sock`) để tạo customer containers. |
...
- **RAM**: Tối thiểu **8 GB**. Docker sử dụng ~2 GB cho hạ tầng; web + worker containers sử dụng ~1 GB; các container kiểm thử tiêu thụ phần còn lại.
- **Ổ đĩa**: ~10 GB trống cho container images và volumes.
- **Git**: Cần có trên máy chủ để quản lý mã nguồn.
- **VS Code** (khuyến nghị): Với Dev Containers extension để có trải nghiệm phát triển tích hợp. Editor thường + trình duyệt cũng hoạt động tốt.

Máy chủ KHÔNG được cài Node.js, npm, pnpm, Python, hoặc bất kỳ runtime nào khác. Tất cả công cụ và runtime đều nằm trong Docker containers.

### Bắt đầu

1. Khởi động tất cả dịch vụ (web, worker, postgres, redis):
   ```bash
   docker compose -f docker/docker-compose.dev.yml up -d
   ```
2. Chạy database migrations (bên trong web container):
   ```bash
   docker compose -f docker/docker-compose.dev.yml exec web npx prisma migrate dev
   ```
3. Web app hiện đang chạy tại `http://localhost:3000` với hot-reload được bật.
4. Xem logs:
   ```bash
   docker compose -f docker/docker-compose.dev.yml logs -f web worker
   ```
5. Tạo bản ghi node trỏ đến Docker daemon cục bộ (worker container mount Docker socket, vì vậy endpoint là đường dẫn socket bên trong container):
   ```
   dockerEndpoint = "unix:///var/run/docker.sock"
   ```

Tất cả lệnh phát triển (`prisma generate`, `prisma migrate`, `npm install`, `npm run test`) đều thực thi bên trong containers thông qua `docker compose exec`. Không có npm, Node.js, hoặc Prisma CLI nào được cài trên máy chủ.

### Container Runtime Giả lập

Để phát triển và kiểm thử nhanh hơn mà không cần Docker, một implementation giả lập của giao diện `ContainerRuntime` (định nghĩa trong `packages/worker/src/runtime/types.ts`) trả về container ID và IP giả lập ngay lập tức — không cần Docker Engine. Việc phát triển chuyển đổi giữa mock và Docker thực thông qua biến môi trường được đặt trong file compose:

```env
CONTAINER_RUNTIME_DRIVER=mock    # hoặc "docker"
```

Cả hai driver đều triển khai cùng một TypeScript interface, vì vậy toàn bộ logic nghiệp vụ, API routes, và phát triển UI có thể tiếp tục bất kể driver nào đang hoạt động. Đây cũng chính là lớp trừu tượng cho phép thay thế Docker bằng một container runtime khác trong tương lai. Khi sử dụng mock driver, không cần mount Docker socket và Docker daemon.
